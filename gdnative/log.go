...
func (l *logger) Println(message ...interface{}) {
	...
	// Print our string.
	C.godot_go_print(API, gdString)

	// Free the string from memory.
	C.godot_go_string_destroy(API, gdString)
}
...
func (l *logger) log(isError bool, message ...interface{}) {
	...
	if isError {
		C.godot_go_print_error(API, cDescription, cFuncName, cFile, cLine)
		return
	}
	C.godot_go_print_warning(API, cDescription, cFuncName, cFile, cLine)
}
...
func stringAsGodotString(value string) *C.godot_string {
	...
	C.godot_go_string_new_with_wide_string(
		API,
		&godotString,
