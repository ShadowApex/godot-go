...
import (
	...
	"github.com/shadowapex/godot-go/godot/class/tileset"
)
...
func (o *TileMap) GetTileset() tileset.TileSet {
	...
	// Call the parent method.
	// TileSet
	retPtr := tileset.NewEmptyTileSet()
	gdnative.MethodBindPtrCall(methodBind, o.GetOwner(), ptrArguments, retPtr)

	// If we have a return type, convert it from a pointer into its actual object.
	ret := tileset.NewTileSetFromPointer(retPtr)
...
func (o *TileMap) SetTileset(tileset tileset.TileSet) {
	log.Println("Calling TileMap.SetTileset()")

	// Build out the method's arguments
	ptrArguments := make([]gdnative.Pointer, 1, 1)
	ptrArguments[0] = gdnative.NewPointerFromObject(tileset.GetOwner())
